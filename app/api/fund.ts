...
export const fetchFundNetValue = async (code: string, date: string): Promise<number | null> => {
...
export const fetchSmartFundNetValue = async (code: string, startDate: string): Promise<{ date: string; value: number } | null> => {
  ...
  return new Promise(async (resolve, reject) => {
    ...
    try {
      await new Promise<void>((resSearch, rejSearch) => {
        (window as Record<string, unknown>)[searchCallbackName] = (data: any) => {
    ...
    tScript.src = tUrl;
    tScript.onload = () => {
      const v = (window as Record<string, unknown>)[`v_jj${c}`] as string | undefined;
      if (v && v.length > 5) {
        const p = v.split('~');
...
export const fetchFundData = async (c: string): Promise<FundData> => {
  ...
  return new Promise(async (resolve, reject) => {
    ...
    (window as any).jsonpgz = (json: JsonpGzData) => {
      ...
      const tencentPromise = new Promise<TencentQuoteData | null>((resolveT) => {
        const tUrl = `https://qt.gtimg.cn/q=jj${c}`;
        const tScript = document.createElement('script');
        tScript.src = tUrl;
        tScript.onload = () => {
        ...
        loadScript(holdingsUrl).then(async () => {
          ...
          for (const r of dataRows) {
            ...
            if (idxName >= 0 && tds[idxName]) {
              ...
              const i = tds.findIndex((txt: string) => txt && txt !== code && !/%$/.test(txt));
              name = i >= 0 ? tds[i] : '';
            }
      ...
      const trendPromise = new Promise<{ historyTrend: TrendPoint[]; yesterdayChange: number | null }>(async (resolveTr) => {
        try {
      ...
      Promise.all([tencentPromise, holdingsPromise, trendPromise]).then(([tData, holdings, trendData]) => {
        ...
        resolve({ ...gzData, holdings, historyTrend, yesterdayChange } as FundData);
      });
    };
    scriptGz.onerror = () => {
      (window as any).jsonpgz = originalJsonpgz;
...
export const fetchShanghaiIndexDate = async (): Promise<string | null> => {
  ...
  return new Promise((resolve, reject) => {
    ...
    script.onload = () => {
      const data = (window as Record<string, unknown>).v_sh000001 as string | undefined;
